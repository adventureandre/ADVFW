 ...
 *
 * @copyright (c) 2016, ANDRE LUIZ FERREIRA DE SOUZA
 */
...
class Seo
{
    ...
    private function getSeo()
    {
        ...
        switch($this->File):
            ...
            case 'single_post':
                ...
                $ReadSeo->ExeRead("adv_posts", "WHERE {$Check} post_name = :link", "link={$this->Link}");
                if(!$ReadSeo->getResult()):
                    ...
                    $this->Data = [$post_title . ' - ' . SITENAME, $post_content, HOME . "/single_post/{$post_name}", HOME . "/uploads/{$post_cover}"];

                    //post:: conta views do post
                    $ArrUpdate = ['post_views' => $post_views + 1];
                    $Update = new Update();
                    $Update->ExeUpdate("adv_posts", $ArrUpdate, "WHERE post_id = :postid", "postid={$post_id}");
                endif;
                break;

            //SEO:: CATEGORIA
            case 'categoria':
                $ReadSeo->ExeRead("adv_categories", "WHERE category_name = :link", "link={$this->Link}");
                if(!$ReadSeo->getResult()):
                    $this->seoData = null;
                    $this->seoTags = null;
                else:
                    ...
                    $this->Data = [$category_title . ' - ' . SITENAME, $category_content, HOME . "/categoria/{$category_name}", INCLUDE_PATH . '/img/site.png'];

                    //category:: conta views da categoria
                    $ArrUpdate = ['category_views' => $category_views + 1];
                    $Update = new Update();
                    $Update->ExeUpdate("adv_categories", $ArrUpdate, "WHERE category_id = :catid", "catid={$category_id}");
                endif;
                break;

            //SEO:: PESQUISA
            case 'pesquisa':
                $ReadSeo->ExeRead("adv_posts", "WHERE post_status = 1 AND (post_title LIKE '%' :link '%' OR post_content LIKE '%' :link '%')", "link={$this->Link}");
                if(!$ReadSeo->getResult()):
                    $this->seoData = null;
                    $this->seoTags = null;
                else:
                    $this->seoData['count'] = $ReadSeo->getRowCount();
                    $this->Data = ["Pesquisa por: {$this->Link}" . ' - ' . SITENAME, "Sua pesquisa por {$this->Link} retornou {$this->seoData['count']} resultados!", HOME . "/pesquisa/{$this->Link}", INCLUDE_PATH . '/img/site.png'];
                endif;
                break;
            ...
            case 'empresas':
                ...
                $this->Data = ["Empresas {$this->Link}" . SITENAME, "Confira o guia completo de sua cidade, e encontra empresas {$this->Link}.", HOME . '/empresas/' . $this->Link, INCLUDE_PATH . '/img/site.png'];
                break;
            ...
            case 'cadastra-empresa':
                $this->Data = ["Cadastre sua Empresa - " . SITENAME, "Página modelo para cadastro de empresas via Front-End do curso Work Series - PHP Orientado a Objetos!", HOME . '/cadastra-empresa/' . $this->Link, INCLUDE_PATH . '/img/site.png'];
                break;

            //SEO:: INDEX
            case 'index':
                $this->Data = [SITENAME . ' - Inicio!', SITEDESC, HOME, INCLUDE_PATH . '/img/site.png'];
                break;

            //SEO:: 404
            default :
                $this->Data = [SITENAME . ' - 404 Oppsss, Nada encontrado!', SITEDESC, HOME . '/404', INCLUDE_PATH . '/img/site.png'];
    ...
    private function setTags()
    {
        ...
        $this->seoTags .= '<meta name="author" content="André Luíz @adventureandre">' . "\n";
        $this->seoTags .= "\n";

        //FACEBOOK
        $this->seoTags .=   '<meta property="fb:app_id" content="514169815386776" />'. "\n";
        ...
        $this->seoTags .= '<meta property="og:app_id" content="'.PG_FACEBOOK_APP.'" />' . "\n";
        $this->seoTags .= '<meta property="article:author" content="https://www.facebook.com/'.PG_FACEBOOK_AUTHOR.'" />' . "\n";
        $this->seoTags .= '<meta property="article:publisher" content="https://www.facebook.com/'.PG_FACEBOOK.'" />' . "\n";
        $this->seoTags .= "\n";
        ...
        $this->seoTags .= '<meta property="twitter:site" content="'.PG_TWITTER.'" />' . "\n";
