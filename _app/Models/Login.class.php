 ...
 * 
 * @copyright (c) 2018, Adventureandre@hotmail.com
 */
class Login {
    ...
    private function setLogin() {
        if (!$this->Email || !$this->Senha || !Check::Email($this->Email)):
            $this->Error = ['Informe seu E-mail e senha para efetuar o login!', ADV_INFOR];
            $this->Result = false;
        elseif (!$this->getUser()):
            $this->Error = ['Os dados informados não são compatíveis!', ADV_ALERT];
            $this->Result = false;
        elseif ($this->Result['user_level'] < $this->Level):
            $this->Error = ["Desculpe {$this->Result['user_name']}, você não tem permissão para acessar esta área!", ADV_ERROR];
            $this->Result = false;
        else:
            $this->Execute();
        endif;
    ...
    private function getUser() {
        ...
        $read = new Read;
        $read->ExeRead("adv_users", "WHERE user_email = :e AND user_password = :p", "e={$this->Email}&p={$this->Senha}");
    ...
    private function Execute() {
        ...
        $this->Error = ["Olá {$this->Result['user_name']}, seja bem vindo(a). Aguarde redirecionamento!", ADV_ACCEPT];
        $this->Result = true;
    }
